...
Статус: обновлённая версия после синхронизации с Foundational Pack и концептуальной базой ARTEMIS v1.0 (2026-04-18).
...
## 15. ПРАВИЛА НАИМЕНОВАНИЯ ФАЙЛОВ ПРИ СОЗДАНИИ

All files created within the ARTEMIS project must follow a strict, deterministic naming convention to ensure clarity, scalability, and consistency across all systems (Drive, GitHub, AI workflows).

Format:
YYYY-MM-DD_AREA_TOPIC_TYPE_STATUS_vMAJOR_MINOR.ext

Rules:

1. The date (YYYY-MM-DD) is mandatory and always placed at the beginning.
2. Use UPPER_SNAKE_CASE for all components after the date.
3. Each filename must include: AREA, TOPIC, TYPE, STATUS, and VERSION.
4. Version must follow the format vMAJOR_MINOR (e.g., v1_0, v4_1).
5. Do not use ambiguous or non-semantic suffixes such as updated, final_version, new, sync, draft2, etc.
6. Do not duplicate semantic meanings (e.g., REPORT_AUDIT).
7. One file = one function. The name must reflect a single clear purpose.
8. Avoid non-structured numbering like 01, 02 unless part of a defined system.
9. File extension must reflect its role:
- .md for system/technical documentation
- .gdoc for active editable documents in Drive
- .pdf for finalized/exported materials

Examples:

2026-04-18_PROJECT_PRIORITIES_PLAN_ACTIVE_v4_1.md
2026-04-18_UIUX_SYSTEM_SPEC_FINAL_v1_0.md
2026-04-05_CORE_MASTER_PROMPT_SYSTEM_ACTIVE_v3_3.gdoc
2026-04-18_RESEARCH_ANALOGS_REPORT_FINAL_v1_0.pdf

Non-compliant filenames must be corrected immediately upon creation or detection.