...
os.environ.setdefault("AUTH_SECRET_KEY", "test-secret-map-feed-draft-feed")
os.environ.setdefault("COOKIE_HTTPONLY", "true")
os.environ.setdefault("COOKIE_SAMESITE", "lax")
os.environ.setdefault("APP_ENV", "development")


class MapFeedDraftFeedTests(unittest.TestCase):
    SERVER_PORT = 8028
    ...
    def setUpClass(cls):
        env = build_clean_test_env(
            {
                "APP_ENV": "development",
                "AUTH_SECRET_KEY": os.environ.get("AUTH_SECRET_KEY", "test-secret-map-feed-draft-feed"),
    ...
    def setUp(self):
        ...
        email = f"draft-feed-{uuid4().hex}@example.com"
    ...
    def test_feed_contains_draft_entities_without_entity_type(self):
    ...
    def test_entity_type_place_returns_422(self):
    ...
    def test_entity_type_and_bbox_filter_order(self):
        ...
        response = self._get_feed(entity_type="draft", bbox="15,5,25,15")
        self.assertEqual(response.status_code, 200, response.text)
        payload = response.json()

        self.assertEqual(payload["total"], 1)
        self.assertEqual(len(payload["items"]), 1)
        self.assertEqual(payload["items"][0]["entity_type"], "draft")
